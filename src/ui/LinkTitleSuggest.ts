...
import { SuggestionItem, CachedFileData, EditorSuggestInternal, SearchMatchReason, PropertyOverFileNamePlugin, VaultInternal, QuickSwitcherPluginInstance, AppInternal, MetadataCacheInternal } from '../types';
...
export class LinkTitleSuggest extends EditorSuggest<SuggestionItem> {
  ...
  private performSearch(query: string): SuggestionItem[] {
    ...
    if (!query || query.trim() === '') {
      ...
      const suggestions: SuggestionItem[] = Array.from(this.fileCache.values())
        .filter(cached => behavior === 'ignore' || !isExcluded(cached.file, this.app))
        // Sort by last modified to show recent files
        .sort((a, b) => b.lastModified - a.lastModified)
        .slice(0, 10) // Limit to 10 for default view
        .map(cached => ({
          file: cached.file,
